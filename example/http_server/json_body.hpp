...
struct json_body
{
    ...
    struct reader
    {
        ...
        {
            // If we know the content-length, we can allocate a monotonic resource to increase the parsing
            // speed. We're using it rather then a static_resource, so a consumer can modify the resulting
            // value. It is also only assumption that the parsed json will be smaller than the serialize one,
