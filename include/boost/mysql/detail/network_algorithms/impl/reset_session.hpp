...
inline error_code process_reset_session_response(
...
struct reset_session_op : boost::asio::coroutine
{
    channel<Stream>& chan_;
    diagnostics& diag_;

    reset_session_op(channel<Stream>& chan, diagnostics& diag) noexcept : chan_(chan), diag_(diag) {}
    ...
    void operator()(Self& self, error_code err = {}, boost::asio::const_buffer buff = {})
    {
        ...
        BOOST_ASIO_CORO_REENTER(*this)
        {
            ...
            serialize_message(reset_connection_packet{}, chan_.current_capabilities(), chan_.shared_buffer());
...
template <class Stream>
void boost::mysql::detail::reset_session(channel<Stream>& chan, error_code& code, diagnostics& diag)
{
    // Serialize the ping message
    serialize_message(reset_connection_packet{}, chan.current_capabilities(), chan.shared_buffer());
    ...
    // Verify it's what we expected
    code = process_reset_session_response(response, chan.current_capabilities(), chan.flavor(), diag);
}

template <class Stream, BOOST_ASIO_COMPLETION_TOKEN_FOR(void(::boost::mysql::error_code)) CompletionToken>
BOOST_ASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void(boost::mysql::error_code))
boost::mysql::detail::async_reset_session(channel<Stream>& chan, diagnostics& diag, CompletionToken&& token)
{
    return boost::asio::async_compose<CompletionToken, void(error_code)>(
        reset_session_op<Stream>(chan, diag),
        token,
        chan
    );
}
