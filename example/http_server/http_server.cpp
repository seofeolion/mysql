...
class server
{
    ...
    net::io_context ioc_;
    mysql::connection_pool db_pool_;
...
public:
        ...
        : num_threads_(threads),
          ...
          db_pool_(
              mysql::connection_params(
                  mysql::handshake_params("orders_user", "orders_password", "boost_mysql_orders"),
                  "127.0.0.1"
              ),
              ioc_.get_executor(),
              1,
    ...
    http::message_generator handle_create_order(net::yield_context yield, const string_request& req)
    {
        ...
        // Issue the query
        mysql::results result;
        conn->async_query("INSERT INTO orders VALUES ()", result, diag, yield[ec]);
        if (ec)
            return internal_server_error(req, "executing query", ec, diag);
    ...
    http::message_generator handle_get_orders(net::yield_context yield, const string_request& req)
    {
        ...
        // Issue the query
        mysql::results result;
        conn->async_query("SELECT id, `status` FROM orders", result, diag, yield[ec]);
        if (ec)
            return internal_server_error(req, "executing query", ec, diag);
    ...
    )
    {
        ...
        // Execute it
        mysql::results result;
        conn->async_execute_statement(stmt, std::make_tuple(order_id), result, diag, yield[ec]);
        if (ec)
            return internal_server_error(req, "executing statement", ec, diag);
    ...
    )
    {
        ...
        conn->async_query("START TRANSACTION", result, diag, yield[ec]);
        if (ec)
        ...
        conn->async_execute_statement(
            stmt,
            std::make_tuple(order_id, *product_id, *quantity),
            result,
            diag,
            yield[ec]
        );
        ...
        // Commit
        conn->async_query("COMMIT", result, diag, yield[ec]);
        if (ec)
    ...
    )
    {
        ...
        conn->async_execute_statement(stmt, std::make_tuple(order_id, line_item_id), result, diag, yield[ec]);
        if (ec)
